...
function cellColor(count: number, max: number) {
  if (count === 0) return "#0c1a0e";
  // Log scale + green ramp: near-black green → deep green → bright green → light green
  const t = Math.log(count + 1) / Math.log(Math.max(max, 1) + 1);
  type RGB = [number, number, number];
  const stops: RGB[] = [
    [12, 26, 14],   // near-black green
    [22, 120, 50],  // deep green
    [34, 197, 94],  // bright green
    [193, 250, 208], // light green
  ];
...
export function Analytics() {
  ...
  const sessionOutcomeSegments = [
    { label: "Completed", value: data?.sessions_by_status?.completed ?? 0, color: "#6b7280" },
    { label: "Active", value: data?.sessions_by_status?.active ?? 0, color: "#16a34a" },
          ...
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                {wsConnected ? (
                  <span className="flex items-center gap-1.5 text-[11px] text-accent bg-accent-muted border border-accent/30 px-2 py-0.5 rounded-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="card p-5">
              ...
              <div className="space-y-4">
                {[
                  { label: "Input", value: data?.tokens.total_input ?? 0, color: "bg-accent" },
                  {
                    label: "Output",
            ...
            <div className="card p-5">
              ...
              <div className="space-y-3">
                {[
                  { label: "Input", value: data?.tokens.total_input ?? 0, color: "text-gray-300" },
                  {
                    label: "Output",
                    value: data?.tokens.total_output ?? 0,
                    color: "text-gray-300",
                  ...
                  <DonutChart
                    segments={costData.breakdown
                      ...
                      .map((b, i) => ({
                        ...
                        color:
                          ["#16a34a", "#22c55e", "#4ade80", "#86efac", "#6b7280", "#9ca3af"][
                            i % 6
                          ] ?? "#6b7280",
                      }))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="card p-5">
              ...
              {dailySessionsLocal.length === 0 ? (
                ...
                <>
                  <Sparkline data={dailySessionsLocal.slice(-30)} color="#16a34a" />
