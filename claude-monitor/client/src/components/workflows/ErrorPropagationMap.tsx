...
const DEPTH_COLORS = ["#16a34a", "#22c55e", "#4ade80", "#9ca3af"];
...
export function ErrorPropagationMap({ data }: ErrorPropagationMapProps) {
  ...
  if (!hasErrors) {
    return (
      <div className="flex flex-col items-center justify-center py-16 gap-3">
        <div className="w-12 h-12 rounded-full bg-accent-muted border border-accent/30 flex items-center justify-center">
          <svg
            width="24"
            height="24"
            viewBox="0 0 24 24"
        ...
        <span className="text-sm text-accent font-medium">No errors recorded</span>
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center gap-3 p-3 rounded-xl bg-gray-500/5 border border-gray-400/15">
        <div className="flex-shrink-0 min-w-[2.75rem] h-10 px-2 rounded-lg bg-gray-500/10 border border-gray-400/20 flex items-center justify-center">
          <span className="text-[13px] font-bold text-gray-300 tabular-nums whitespace-nowrap">
            {errorRatePct}%
          </span>
        </div>
            ...
            {topTypes.map((t, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[11px] font-semibold text-gray-300 tabular-nums w-5 text-right flex-shrink-0">
                    {t.count}
                  </span>
                </div>
              );
        ...
        <div>
          ...
          <div className="flex flex-col gap-1">
            {data.eventErrors.map((e) => (
              <div
                key={e.summary}
                className="flex items-center gap-2.5 px-2.5 py-2 rounded-lg bg-gray-500/5 border border-gray-400/10 hover:border-gray-400/20 transition-colors"
              >
                <svg
                  width="14"
                  height="14"
                ...
                <span className="flex-shrink-0 text-[11px] font-semibold text-gray-400 tabular-nums">
                  {e.count}x
                </span>
              </div>
            ))}
