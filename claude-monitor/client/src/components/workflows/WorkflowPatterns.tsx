...
function StepPill({ label }: { label: string }) {
  return (
    <span className="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium bg-gray-500/15 text-gray-300 border border-gray-500/20 whitespace-nowrap">
      {label}
    </span>
  );
}
...
function PatternItem({ pattern, rank, isSelected, onClick }: PatternItemProps) {
  ...
  return (
    ...
    >
      {/* Rank / icon */}
      <div className="flex-shrink-0 w-7 h-7 rounded-md bg-gray-500/10 border border-gray-500/20 flex items-center justify-center">
        {rank <= 3 ? (
...
function SoloSessionItem({ count, percentage }: { count: number; percentage: number }) {
  return (
    <div className="flex items-center gap-3 px-4 py-3 rounded-lg border bg-gray-500/5 border-gray-500/20">
      <div className="flex-shrink-0 w-7 h-7 rounded-md bg-gray-500/10 border border-gray-500/20 flex items-center justify-center">
        <Zap className="w-3.5 h-3.5 text-gray-400" />
      </div>
      <div className="flex-1 min-w-0">
        <span className="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium bg-gray-500/15 text-gray-300 border border-gray-500/20">
          solo (no subagents)
        </span>
      </div>
      <div className="flex-shrink-0 text-right">
