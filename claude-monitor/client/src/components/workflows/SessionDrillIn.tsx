...
function statusColor(status: string): string {
  switch (status) {
    ...
    case "working":
      return "text-accent bg-accent-muted border-accent/30";
    case "error":
      return "text-gray-200 bg-gray-500/10 border-gray-400/30";
    case "active":
      return "text-accent bg-accent-muted border-accent/30";
    case "idle":
      return "text-gray-400 bg-gray-500/10 border-gray-500/20";
  ...
  return (
    <div>
      ...
      >
        ...
        {/* Name */}
        <span
          className={`text-sm font-medium truncate ${isMain ? "text-gray-200" : "text-gray-200"}`}
        >
          {node.name}
        </span>
    ...
    <div className="overflow-x-auto max-h-[420px] overflow-y-auto">
      <div className="flex flex-col gap-1 min-w-0">
        {events.map((ev) => (
          ...
          >
            {/* Tool pill */}
            <span className="flex-shrink-0 inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium bg-gray-500/15 text-gray-300 border border-gray-500/20 whitespace-nowrap">
...
const EVENT_TYPE_COLOR: Record<string, string> = {
  tool_use: "text-gray-400",
  tool_result: "text-accent",
  agent_start: "text-gray-300",
  agent_stop: "text-gray-400",
  compaction: "text-gray-400",
  error: "text-gray-300",
};
...
function ErrorState({ message }: ErrorStateProps) {
  return (
    <div className="flex flex-col items-center justify-center py-10 text-center px-4">
      <div className="w-9 h-9 rounded-xl bg-gray-500/10 border border-gray-400/20 flex items-center justify-center mb-3">
        <X className="w-4 h-4 text-gray-400" />
      </div>
      <p className="text-sm font-medium text-gray-300">Failed to load session</p>
    ...
    <div ref={containerRef} className="relative mb-4">
      ...
      <div
        className={[
          ...
          open ? "border-gray-500/40 ring-1 ring-gray-500/20" : "border-border",
        ].join(" ")}
        onClick={() => {
          setOpen(true);
