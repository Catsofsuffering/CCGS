---
description: '需求 -> 约束集合（Codex 主导研究，Gemini 可选补充）'
---
...
- Gemini is an optional enhancement, not a prerequisite for the primary workflow.
...
- When guiding the user, always reference `/ccg:*` commands rather than `/opsx:*`.
...
7. **Finalize the Proposal Context**
   ...
   - Stop after proposal generation and direct the user to `/ccg:spec-plan`.

8. **Context Checkpoint**
   - Report current context usage.
   - If context is getting large, suggest clearing before `/ccg:spec-plan`.
